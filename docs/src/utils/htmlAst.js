...
const fetchChildType = (code_block) => {
  if (code_block.children[0].properties.className === undefined) return false;
  const dataMeta = code_block.children[0].properties.dataMeta;
  const lang = code_block.children[0].properties.className[0];
  return dataMeta ? `${lang}-${dataMeta}` : `${lang}`;
};
...
export const getLinks = (ast) => {
  const headers = ast.children.filter(
    (el) => el.type === "element" && _.includes(["h1", "h2"], el.tagName)
  );
